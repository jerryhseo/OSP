...
<liferay-portlet:renderURL var="virtualLabClassManagementDetailURL">
	<liferay-portlet:param name="myRender" value="virtualLabClassManagementDetail" />
</liferay-portlet:renderURL>
				...
				for(var i = 0; i < virtualLabList.length; i++) {
					...
					if (tempVirtualLabId == virtualLabList[i].virtualLabId){
						...
						if("${isAdministrator}"=="true" || "${isSiteAdministrator}"=="true") {
							...
							if(virtualLabList[i].virtualLabManagerYn || virtualLabList[i].virtualLabOwnerYn){
								$("<td/>").text(virtualLabList[i].virtualLabTitle)
								  .css("text-align","center")
								  .css("cursor","pointer")
								  .css("color", "#29547e")
	...
	jQuery.ajax({
		...
		success: function(msg) {
			...
			if(msg.result == "ADMINISTRATOR") {
				var virtualLabClassManagementDetailURL = "<%=virtualLabClassManagementDetailURL%>"
					window.location.href = virtualLabClassManagementDetailURL + "&classId=" + classId;
			}else {
...
function <portlet:namespace/>moveVirtualLab(virtualLabId) {
	...
	window.location.href = virtualLabManagementDetailURL + "&<portlet:namespace/>virtualLabId=" + virtualLabId;
}
...
<div id="virtualLab-request-dialog" title="<liferay-ui:message key='edison-virtuallab-request-infomation' />" class="newWindow" style="background-color: #fff; display:none;">
	...
	<form id="createVirtualLabForm" name="createVirtualLabForm" method="post" action="<%= virtualLabCreateURL %>" onsubmit="return <portlet:namespace/>checkValidation(this);" style="margin:0px;">
		...
		<div style="text-align: right; margin:0px 25px 30px 0px;">
			<c:choose>
				<c:when test="${role eq 'LABOWNER' }">
					<input id="virtualLab_creation_button" name="virtualLab_creation_button" type="submit" value="<liferay-ui:message key='edison-virtuallab-creation' />" class="btn btn-default" />
				</c:when>
				<c:otherwise>
					<input id="virtualLab_creation_button" name="virtualLab_creation_button" type="submit" value="<liferay-ui:message key='edison-virtuallab-creation-request' />" class="btn btn-default" />
				</c:otherwise>
			</c:choose>
		</div>
	</form>
...
<div id="request-denied-dialog" title="<liferay-ui:message key='edison-virtuallab-virtualLabClassRegistrationList-course-registration' />" class="newWindow" style="background-color: #fff; display:none;" >
	...
	<div style="text-align: right; margin:0px 25px 30px 0px;">
		<input id="register_delete_button" name="register_delete_button" class="btn btn-default" type="button" onclick="deleteVirtualLabRequest()" value="<liferay-ui:message key='edison-virtuallab-cancel-request' />" />
	</div>
</div>
