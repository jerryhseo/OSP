...
var WorkboxSW = (function() {
    ...
    class LogHelper {
        ...
        _getAllLogGroups(a, b) {
            ...
            return b.data && ('object' != typeof b.data || b.data instanceof Array ? e.addLog(this._getKeyValueDetails('additionalData', b.data)) : Object.keys(b.data).forEach((a) => {
    ...
    function atLeastOne(a) {
        ...
        b.some((b) => a[b] !== void 0) || throwError('Please set at least one of the following parameters: ' + b.map((a) => `'${a}'`).join(', '));
    }
    ...
    function tokensToFunction(a) {
        for (var b = Array(a.length), c = 0; c < a.length; c++) 'object' == typeof a[c] && (b[c] = new RegExp('^(?:' + a[c].pattern + ')$'));
        return function(c, d) {
            for (var e, f = '', g = c || {}, h = d || {}, k = h.pretty ? encodeURIComponentPretty : encodeURIComponent, l = 0; l < a.length; l++) {
                ...
                if (index$1(m)) {
                    if (!e.repeat) throw new TypeError('Expected "' + e.name + '" to not repeat, but received `' + JSON.stringify(m) + '`');
                    if (0 === m.length)
                        if (e.optional) continue;
                        else throw new TypeError('Expected "' + e.name + '" to not be empty');
                    for (var n = 0; n < m.length; n++) {
                        if (i = k(m[n]), !b[l].test(i)) throw new TypeError('Expected all "' + e.name + '" to match "' + e.pattern + '", but received `' + JSON.stringify(i) + '`');
                        f += (0 === n ? e.prefix : e.delimiter) + i;
                    }
                    continue
                }
                if (i = e.asterisk ? encodeAsterisk(m) : k(m), !b[l].test(i)) throw new TypeError('Expected "' + e.name + '" to match "' + e.pattern + '", but received "' + i + '"');
                f += e.prefix + i;
            }
            return f
        }
    ...
    function arrayToRegexp(a, b, c) {
        for (var d = [], e = 0; e < a.length; e++) d.push(pathToRegexp(a[e], b, c).source);
    ...
    function tokensToRegExp(a, b, c) {
        ...
        for (var d, e = c.strict, f = !1 !== c.end, g = '', h = 0; h < a.length; h++)
            if (d = a[h], 'string' == typeof d) g += escapeString(d);
            else {
                var i = escapeString(d.prefix),
                    j = '(?:' + d.pattern + ')';
                b.push(d), d.repeat && (j += '(?:' + i + j + ')*'), j = d.optional ? d.partial ? i + '(' + j + ')?' : '(?:' + i + '(' + j + '))?' : i + '(' + j + ')', g += j;
            }
        var k = escapeString(c.delimiter || '/'),
            l = g.slice(-k.length) === k;
        return e || (g = (l ? g.slice(0, -k.length) : g) + '(?:' + k + '(?=$))?'), g += f ? '$' : e && l ? '' : '(?=' + k + '|$)', attachKeys(new RegExp('^' + g, flags(c)), b)
    }
            ...
            super({
                match: ({
                    ...
                    if ('navigate' === d.request.mode) {
                        ...
                        a.some((a) => a.test(d)) ? b.some((a) => a.test(d)) ? f = `The navigation route is not being used, since the ` + `request URL matches both the whitelist and blacklist.` : (f = `The navigation route is being used.`, g = !0) : f = `The navigation route is not being used, since the ` + `URL being navigated to doesn't match the whitelist.`, logHelper.debug({
                            that: this,
                            message: f,
                            data: {
                                "request-url": e.href,
        ...
        constructor({
            ...
            super({
                match: ({
                    ...
                    return c ? b.origin !== location.origin && 0 !== c.index ? (logHelper.debug({
                        that: this,
    ...
    class Router$2 {
        ...
        addFetchListener() {
            return this._isListenerRegistered ? (logHelper.warn({
                that: this,
                message: `addFetchListener() has already been called for this Router.`
            }), !1) : (this._isListenerRegistered = !0, self.addEventListener('fetch', (a) => {
                const b = this.handleRequest({
                    event: a
                });
                b && a.respondWith(b);
            }), !0)
        ...
        handleRequest({
            ...
            if (!c && this.defaultHandler && (c = this.defaultHandler), c) {
                ...
                return this.catchHandler && (e = e.catch((c) => this.catchHandler.handle({
                    url: b,
                    event: a,
                    error: c
                }))), e
        ...
        _findHandlerAndParams({
            ...
            for (const d of c) {
                ...
                }), Array.isArray(c) && 0 === c.length ? c = void 0 : c.constructor === Object && 0 === Object.keys(c).length && (c = void 0), {
                    params: c,
                    handler: d.handler
                }
            }
        ...
        unregisterRoutes({
            ...
            routes registered.`,
                ...
                const a = this._routes.get(b.method).indexOf(b); - 1 < a ? this._routes.get(b.method).splice(a, 1) : logHelper.error({
                    that: this,
    ...
    class Router$$1 extends Router$2 {
        ...
        registerRoute(a, b, c = 'GET') {
            ...
            if ('string' == typeof a) {
                ...
                const e = a.startsWith('http') ? new URL(a, location).pathname : a,
                    f = e.match(/[*:?+]/);
                f && logHelper.warn({
                    message: `registerRoute() was called with a string containing an ` + `Express-style wildcard character. In the next version of ` + `Workbox, Express-style wildcards won't be supported, and ` + `strings will be treated a exact matches. Please switch to ` + `regular expressions for equivalent behavior.`,
                    data: {
                        "Path String": a,
                        "Wildcard Character": f[0],
    ...
    const errors$2 = {
        ...
        "multiple-cached-response-will-be-used-plugins": 'You cannot register more than one plugin that implements cachedResponseWillBeUsed.',
    ...
    class CacheableResponse {
        ...
        isResponseCacheable({
            ...
            let c = !0;
            if (this.statuses && (c = this.statuses.includes(b.status)), this.headers && c && (c = Object.keys(this.headers).some((a) => b.headers.get(a) === this.headers[a])), !c) {
                const c = {
                    response: b
                };
                this.statuses && (c['valid-status-codes'] = JSON.stringify(this.statuses)), this.headers && (c['valid-headers'] = JSON.stringify(this.headers)), a && (c.request = a), logHelper.debug({
    ...
    var cleanResponseCopy = (({
        ...
        return c.then((a) => new Response(a, {
            headers: b.headers,
            status: b.status,
    ...
    class RequestWrapper {
            ...
            a ? (isType({
                ...
                for (let b of pluginCallbacks)
                    if ('function' == typeof a[b]) {
                        ...
                        else if ('cacheWillUpdate' === b) throw ErrorFactory$4.createError('multiple-cache-will-update-plugins');
                        else if ('cachedResponseWillBeUsed' === b) throw ErrorFactory$4.createError('multiple-cached-response-will-be-used-plugins');
        ...
        getCache() {
            ...
            return asyncToGenerator(function*() {
                return a._cache || (a._cache = yield caches.open(a.cacheName)), a._cache
            })()
        }
        match({
            request: a
            ...
            return asyncToGenerator(function*() {
                ...
                if (b.plugins.has('requestWillFetch'))
                    for (let c of b.plugins.get('requestWillFetch')) {
                        const b = yield c.requestWillFetch({
                            request: a
                        });
                        isInstance({
                            returnedRequest: b
        ...
        fetchAndCache({
            ...
            return asyncToGenerator(function*() {
                ...
                let g;
                const h = yield f.fetch({
                    request: a
                }), i = f._userSpecifiedCachableResponsePlugin || d || f.getDefaultCacheableResponsePlugin(), j = yield i.cacheWillUpdate({
                    request: a,
                    response: h
                });
                if (j) {
                    const b = e && h.redirected ? yield cleanResponseCopy({
                        response: h
                    }): h.clone();
                    g = f.getCache().then((() => {
                        var d = asyncToGenerator(function*(d) {
                            let e;
                            const g = c || a;
                            if ('opaque' !== h.type && f.plugins.has('cacheDidUpdate') && (e = yield f.match({
                                    request: g
                                })), yield d.put(g, b), f.plugins.has('cacheDidUpdate'))
                                for (let a of f.plugins.get('cacheDidUpdate')) yield a.cacheDidUpdate({
    ...
    class CacheFirst extends Handler {
        handle({
            ...
            return asyncToGenerator(function*() {
                ...
                const c = yield b.requestWrapper.match({
                    request: a.request
                });
                return c || (yield b.requestWrapper.fetchAndCache({
                    request: a.request,
    ...
    class CacheOnly extends Handler {
        handle({
            ...
            return asyncToGenerator(function*() {
                return isInstance({
                    event: a
                }, FetchEvent), yield b.requestWrapper.match({
                    request: a.request
                })
            })()
        }
            ...
            return asyncToGenerator(function*() {
                ...
                b.networkTimeoutSeconds && c.push(new Promise(function(c) {
                    d = setTimeout(function() {
                        c(b.requestWrapper.match({
                            request: a.request
                        }));
                    }, 1e3 * b.networkTimeoutSeconds);
                }));
                const e = b.requestWrapper.fetchAndCache({
                    request: a.request,
                    waitOnCache: b.waitOnCache,
    ...
    class NetworkOnly extends Handler {
        handle({
            ...
            return asyncToGenerator(function*() {
                return isInstance({
                    event: a
                }, FetchEvent), yield b.requestWrapper.fetch({
                    request: a.request
                })
            })()
        }
    ...
    class StaleWhileRevalidate extends Handler {
        ...
        handle({
            ...
            return asyncToGenerator(function*() {
                ...
                const c = b.requestWrapper.fetchAndCache({
                        request: a.request,
                        waitOnCache: b.waitOnCache,
                        cacheResponsePlugin: b._cacheablePlugin
                    }).catch(function() {
                    ...
                    d = yield b.requestWrapper.match({
                        request: a.request
                    });
                return d || (yield c)
            })()
    ...
    class CacheExpiration {
        ...
        expireEntries({
            ...
            return asyncToGenerator(function*() {
                ...
                c._expirationMutex = !0, isType({
                    cacheName: a
                }, 'string'), 'undefined' == typeof b && (b = Date.now());
                const d = c.maxAgeSeconds ? yield c.findOldEntries({
                    cacheName: a,
                    now: b
                }): [], e = c.maxEntries ? yield c.findExtraEntries({
                    cacheName: a
                }): [], f = [...new Set(d.concat(e))];
                if (yield c.deleteFromCacheAndIDB({
                        cacheName: a,
                        urls: f
                    }), 0 < f.length && logHelper.debug({
                        that: c,
                    ...
                    }), c._expirationMutex = !1, c._timestampForNextRun) {
                    const b = c._timestampForNextRun;
                    return c._timestampForNextRun = null, c.expireEntries({
                        cacheName: a,
                        now: b
                    })
                }
        ...
        findOldEntries({
            ...
            return asyncToGenerator(function*() {
                ...
                const d = b - 1e3 * c.maxAgeSeconds,
                    ...
                    f = yield c.getDB({
                        cacheName: a
                    }), g = f.transaction(a, 'readonly'), h = g.objectStore(a), i = h.index(timestampPropertyName);
                return i.iterateCursor(function(a) {
        ...
        findExtraEntries({
            ...
            return asyncToGenerator(function*() {
                ...
                return h > b.maxEntries && (e = d.transaction(a, 'readonly'), f = e.objectStore(a), g = f.index(timestampPropertyName), g.iterateCursor(function(a) {
                    a && (c.push(a.value[urlPropertyName]), h - c.length > b.maxEntries && a.continue());
                })), yield e.complete, c
            })()
        }
        deleteFromCacheAndIDB({
            cacheName: a,
            urls: b
        } = {}) {
            var c = this;
        ...
        cacheDidUpdate({
            ...
            return asyncToGenerator(function*() {
                isType({
                    ...
                    newResponse: b
                }, Response), 'undefined' == typeof d && (d = Date.now()), yield e.updateTimestamp({
                    cacheName: a,
                    url: c,
                    now: d
                }), yield e.expireEntries({
                    cacheName: a,
                    now: d
                });
            })()
    ...
    function responsesAreSame({
        ...
        return d ? c.every((c) => a.headers.has(c) === b.headers.has(c) && a.headers.get(c) === b.headers.get(c)) : (logHelper.log({
    ...
    class Strategies {
        ...
        _getCachingMechanism(a, b = {}) {
            ...
            b.excludeCacheId || (d.cacheId = this._cacheId), b.cacheName && (d.cacheName = b.cacheName);
    ...
    class BaseCacheManager {
        ...
        _addEntries(a) {
            this._parsedCacheUrls = null, a.forEach((a) => {
        ...
        install() {
            ...
            return asyncToGenerator(function*() {
                ...
                const b = [];
                return a._entriesToCache.forEach(function(c) {
        ...
        _cacheEntry(a) {
            var b = this;
            return asyncToGenerator(function*() {
                const c = yield b._isAlreadyCached(a), d = {
                    url: a.request.url,
                    revision: a.revision,
                    wasUpdated: !c
                };
                if (c) return d;
                try {
                    return yield b._requestWrapper.fetchAndCache({
                        request: a.getNetworkRequest(),
                        waitOnCache: !0,
                        cacheKey: a.request,
                        cleanRedirects: !0
                    }), yield b._onEntryCached(a), d
                } catch (b) {
        ...
        cleanup() {
            ...
            return asyncToGenerator(function*() {
                ...
                const c = yield a._getCache(), d = yield c.keys(), e = d.filter(function(a) {
        ...
        _getCache() {
            var a = this;
            return asyncToGenerator(function*() {
                return a._cache || (a._cache = yield caches.open(a.getCacheName())), a._cache
            })()
        }
        _parseEntry() {
    ...
    class IDBHelper {
        ...
        get(a) {
            return this._getDb().then((b) => b.transaction(this._storeName).objectStore(this._storeName).get(a))
        }
        getAllValues() {
            return this._getDb().then((a) => a.transaction(this._storeName).objectStore(this._storeName).getAll())
        }
        getAllKeys() {
            return this._getDb().then((a) => a.transaction(this._storeName).objectStore(this._storeName).getAllKeys())
        }
    }
        ...
        getNetworkRequest() {
            ...
            if (!0 === this.cacheBust)
                if ('cache' in Request.prototype) b.cache = 'reload';
                    ...
                    b.search += (b.search ? '&' : '') + encodeURIComponent(cacheBustParamName) + '=' + encodeURIComponent(this.revision), a = b.toString();
                }
    ...
    class ObjectCacheEntry extends BaseCacheEntry {
        constructor({
            ...
            else if (0 === a.length) throw new WorkboxError('invalid-object-entry', {
    ...
    class RevisionedCacheManager extends BaseCacheManager {
        ...
        _isAlreadyCached(a) {
            ...
            return asyncToGenerator(function*() {
                ...
                const d = yield b._getCache(), e = yield d.match(a.request);
                return !!e
            })()
        }
        _onEntryCached(a) {
        ...
        install() {
            return super.install().then((a) => {
                ...
                const d = {};
                return 0 < b.length && (d['New / Updated Precache URL\'s'] = this._createLogFriendlyString(b)), 0 < c.length && (d['Up-to-date Precache URL\'s'] = this._createLogFriendlyString(c)), logHelper.log({
                    message: `Precache Details: ${b.length} requests ` + `were added or updated and ` + `${c.length} request are already ` + `cached and up-to-date.`,
                    data: d
                }), a
            })
        }
    ...
    class WorkboxSW$1 {
        constructor({
          ...
          to the smaller, optimized production build of Workbox.`)), a && ('string' != typeof a || 0 === a.length)) throw ErrorFactory.createError('bad-cache-id');
            ...
            f && h.push(new BroadcastCacheUpdatePlugin({
                ...
                cacheId: a
            }), this._precacheRouter = new Router$$1(this._revisionedCacheManager.getCacheName()), this._router = new Router$$1(this._revisionedCacheManager.getCacheName()), d && (this._precacheRouter.addFetchListener(), this._router.addFetchListener()), this._registerInstallActivateEvents(b, c), this._registerDefaultRoutes(g, e);
        }
        precache(a) {
        ...
        _registerInstallActivateEvents(a, b) {
            self.addEventListener('install', (b) => {
                ...
                0 < c.length && logHelper.debug({
                    that: this,
                    message: `The precached URLs will automatically be served using a
            cache-first strategy.`,
                    data: {
                        "Precached URLs": JSON.stringify(c)
                    }
                }), b.waitUntil(this._revisionedCacheManager.install().then(() => {
                    if (a) return self.skipWaiting()
                }));
            }), self.addEventListener('activate', (a) => {
                a.waitUntil(this._revisionedCacheManager.cleanup().then(() => {
        ...
        _registerDefaultRoutes(a, b) {
            ...
            this._precacheRouter.registerRoute(({
                url: c
            }) => {
                c.hash = '';
                ...
                return -1 !== d.indexOf(e.href) || b && e.pathname.endsWith('/') && (e.pathname += b, -1 !== d.indexOf(e.href))
            }, d);
        }
