...
console.log('[ParaviewGlance] file explorer id Check : '+<portlet:namespace/>fileExplorerId);
 ...
 ***********************************************************************/
 ...
 function <portlet:namespace/>downloadCurrentFile(){
	console.log("[ParaviewGlance] download Request.");
		...
		reader.onload = function (e) {
			$('#<portlet:namespace/>canvas').each(function(){
				$(this).prop('contentWindow').loadJSMolFile(e.target.result);
					console.log("[ParaviewGlance] local file load event 1 : ", e.target);
					console.log("[ParaviewGlance] local file load event 2 : ", e.target.result);
			});
	        console.log("[ParaviewGlance] local file test : ", input);
			console.log("[ParaviewGlance] local file test2 : ", input.files[0]);
...
function openFileExplorerfromParaviewGlanceOSP(){
	console.log("[ParaviewGlance]test openserver menu");
...
Liferay.on(
  	...
  	function(e){
  		...
  		console.log('[ParaviewGlance]OSP_HANDSHAKE: ['+e.portletId+', '+new Date()+']');
...
Liferay.on(
	...
	function(e){
		var myId = '<%=portletDisplay.getId()%>';
		if(myId !== e.targetPortlet) return; 
		console.log('[ParaviewGlance]Regestered'+e.portletId+' activated. '+new Date()+']', e);
	}
);
...
Liferay.on( 
	...
	function(e){
		...
		console.log('[ParaviewGlance]OSP_LOAD_DATA: ['+e.portletId+', '+new Date()+']', e.data);
...
Liferay.on(
	...
	function( e ){
		...
		console.log('[ParaviewGlance] OSP_RESPONSE_DATA: ['+e.portletId+', '+new Date()+']');

		var inputData = new OSP.InputData( e.data );

		if( inputData.type() !== OSP.Enumeration.PathType.FILE ){
			alert( '[ParaviewGlance]File should be selected!' );
			return;
		}
		else{
...
Liferay.on(
	...
	function(e){
		...
		console.log('[ParaviewGlance]OSP_REFRESH_OUTPUT_VIEW: ['+e.portletId+', '+new Date()+']');
...
Liferay.on(
	...
	function(e){
		...
		console.log('[ParaviewGlance]OSP_INITIALIZE: ['+e.portletId+', '+new Date()+']', e);
		
		$("#<portlet:namespace/>canvas").attr('src', '<%=request.getContextPath()%>/html/ospparaviewglance/load_jsmol.jsp');
...
function <portlet:namespace/>loadJSMolFile( inputData ){
	console.log("[ParaviewGlance] Load Data : input Data ", inputData);
...
function <portlet:namespace/>drawJSMol( inputData ){
    setTimeout(
	    function(){
	    	...
	    	var iframeDoc = iframe.contentDocument || iframe.contentWindow.loadFileOnParaviewGlance;

	    	console.log( '[ParaviewGlance]iframeDoc.readyState : ', iframeDoc.readyState);
	    	if (  iframeDoc.readyState  == 'complete' && iframe.contentWindow.loadFileOnParaviewGlance ) {
	    		
	    		console.log("[ParaviewGlance] test 1 : load data in drawJSMol");
	    	    AUI().use('liferay-portlet-url', function(a) {
	    	        ...
	    	        console.log( '[ParaviewGlance]Draw JSMol: ', inputData);
	    	        
		    	    iframe.contentWindow.loadFileOnParaviewGlance(inputData, serveResourceUrl.toString());
		    	    
		    	    $('#<portlet:namespace/>title').html(inputData.name());
	    	    });
	    	} 
	    	else{
	    		console.log("[ParaviewGlance] test esle : load data in drawJSMol");
...
function <portlet:namespace/>getFirstFileName( inputData ){
	console.log('[ParaviewGlance]get First File Name : ', inputData );
    ...
    console.log("[ParaviewGlance] laod get first file test : ", data);
	...
	$.ajax({
		...
		success: function(data) {
			console.log("[ParaviewGlance] get result data ", data);
			inputData.name( data.fileName );
			inputData.type( OSP.Enumeration.PathType.FILE );
			<portlet:namespace/>drawJSMol( inputData );
			console.log("[ParaviewGlance] Get First File Data : ", inputData);
		},
		error:function(data,e){
			console.log('[ParaviewGlance]AJAX ERROR1-->', data);
			console.log('[ParaviewGlance]AJAX ERROR2-->', e);
		},
		complete: function( jqXHR, textStatus ){
			console.log('[ParaviewGlance]AJAX complete ', jqXHR);
		}
	});
}
...
function <portlet:namespace/>downloadCurrentFile(){
	console.log("[ParaviewGlance] Download current data", <portlet:namespace/>currentData);
...
function <portlet:namespace/>initialize( inputData ){
	...
	if( $.isEmptyObject( inputData ) ){
		...
		console.log('[ParaviewGlance] initialize input data', inputData);
