(function(window){
	...
	OSP.Layout = function( jsonLayout ){
					...
					AUI().use('liferay-portlet-url', function(A){
						...
						$.ajax({
							...
							success: function( renderResult ){
								var $portletDiv = $('<div>').prop('id', P.getNamespace());
		...
		var Column = function( jsonColumn ){
			...
			C.loadPortlet = function( portletId, connector, eventEnable, windowState, callback ){
				//console.log( 'Target Portlet ID: '+portletId);
				var targetPortlet = C.getPortlet(portletId);
			...
			C.switchDisplayPortlet = function( targetId/* or port name*/, connector, eventEnable, windowState, callback){
				...
				var $targetDiv = $('#'+C.getPortletSectionId(connector));
				var targetPortlet = C.getPortlet(targetId);
				C.loadPortlet( targetPortlet.instanceId(), connector, eventEnable, windowState, callback);
		...
		Layout.loadPortlets = function( workbenchId, eventEnable, windowState, callback ){
			var columns = Layout.columns();
			if( !columns )	return false;
			...
			for( var index in columns ){
				...
				if( !portletId ){
					...
					console.log('PortName: '+targetPortlet.portName());
					console.log('PortletId: '+targetPortlet.instanceId());
				}
				else
					targetPortlet = column.getPortlet(portletId);
				
				column.loadPortlet( 
							targetPortlet.instanceId(), 
							workbenchId, 
							eventEnable,
							windowState,
							callback 
		...
		Layout.switchDisplayColumnPortlet = function( toPortletId /* or port name */, connector, eventEnable, windowState, callback ){
			...
			return column.switchDisplayPortlet(toPortletId, connector, eventEnable, windowState, callback);
		};
	...
	OSP.Workbench = function( jsonWorkbench ){
		...
		Workbench.loadPortlets = function( eventEnable, windowState, callback ){
			...
			layout.loadPortlets( Workbench.id(), eventEnable, windowState, callback );
		};
