package com.kisti.osp.analyzer.portlet.common;
...
/**
 * Portlet implementation class DefaultViewerPortlet
 */
public class DefaultViewerPortlet extends MVCPortlet {
  private static Log _log = LogFactoryUtil.getLog(DefaultViewerPortlet.class);
    ...
    if(command.equalsIgnoreCase("READ_FILE")){
      try{
        FileManagementLocalServiceUtil
          .readFileContent(resourceRequest, resourceResponse, filePath.toString(), isJobResult);
      ...
      try{
        FileManagementLocalServiceUtil
          .readFirstFileContent(resourceRequest, resourceResponse, parentPath, fileName, isJobResult);
      }catch (PortalException | SystemException e){
        _log.error("readFileContent(): " + filePath.toString());
        throw new PortletException();
      }
        ...
        try{
            FileManagementLocalServiceUtil
              .getFirstFileName(resourceRequest, resourceResponse, parentPath, fileName, isJobResult);
          }catch (PortalException | SystemException e){
      ...
      try{
        FileManagementLocalServiceUtil
          .saveFileContent(resourceRequest, filePath.toString(), context, isJobResult);
      }catch (PortalException | SystemException e){
      ...
      try{
          FileManagementLocalServiceUtil
          .checkValidFile(resourceRequest, resourceResponse, filePath.toString(), isJobResult);
      }catch (PortalException | SystemException e){
      ...
      try{
        FileManagementLocalServiceUtil
          .duplicated(resourceRequest, resourceResponse, filePath.toString(),isJobResult);
      }catch (PortalException | SystemException e){
      ...
      try{
        ...
        try{
            FileManagementLocalServiceUtil
                .download(resourceRequest, resourceResponse, parentPath, new String[]{fileName}, isJobResult);
        }catch (PortalException | SystemException e){
          _log.error("checkValidFile(): " + filePath.toString());
          throw new PortletException();
        }
