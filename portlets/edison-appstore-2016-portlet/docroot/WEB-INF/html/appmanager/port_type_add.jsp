...
<script src="<c:url value="/js/osp_ports.js" />"></script>
...
<%
	...
	String targetLanuage = GetterUtil.get(request.getAttribute("targetLanuage"), "");
	String defaultLanguage = GetterUtil.get(request.getAttribute("defaultLanguage"), "");
	String availableLanguages = GetterUtil.get(request.getAttribute("availableLanguages"), "");
	
	String portletQueryStr = "&targetLanuage="+targetLanuage+"&defaultLanguage="+defaultLanguage+"&availableLanguages="+availableLanguages;
%>
	...
	<div class="newWcont01">
		...
		<c:if test="${inputDeckExist}">
			<div style="min-height: 700px;" class="inputdeck-portlet">
				<liferay-portlet:runtime portletName="edisoninputDeck_WAR_edisonappstore2016portlet" queryString="<%=portletQueryStr%>"/>
	...
	if(confirm(Liferay.Language.get('would-you-like-to-save'))){
		...
		jQuery.ajax({
			...
			success: function(msg) {
				...
				$("#port-type-add-dialog").dialog("close");
				<portlet:namespace/>portAddOpen('${scienceAppId}','${portType}','${prePortName}','${portTypeName}');
			},error:function(jqXHR, textStatus, errorThrown){
				if(jqXHR.responseText !== ''){
					alert(textStatus+": "+jqXHR.responseText);
				}else{
