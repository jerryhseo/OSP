...
<%
	...
	String dialogId = request.getAttribute("dialogId")!= null?(String)request.getAttribute("dialogId"):"";
	
	String portName = request.getAttribute("portName")!= null?(String)request.getAttribute("portName"):"";
	String taskId = request.getAttribute("taskId")!= null?(String)request.getAttribute("taskId"):"";
%>
...
<div class="newWindow" style="background-color: #fff; overflow-x: hidden;overflow-y: hidden;">
	...
	<aui:form method="post" name="fileUploadForm" enctype="multipart/form-data" action="<%=fileIBUploadURL %>" >
		<aui:input type="hidden" name="destDialogId" value="${dialogId}"></aui:input>
...
<%-- 						<aui:input name="file" name="<portlet:namespace/>addFile" id="<portlet:namespace/>file0"></aui:input> --%>
					...
					<div class="mfp03">
						<input class="addIp button02_1" onClick="submitCheck();" value="<liferay-ui:message key='edison-button-upload'/>" type="button">
					</div>
			 </div>
		</div>
