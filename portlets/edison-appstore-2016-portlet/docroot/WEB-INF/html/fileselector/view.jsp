...
<%
	...
	String popUpWidth =request.getParameter("width")!= null?(String)request.getParameter("width").toString():"800";
...
<liferay-portlet:renderURL var="fileUploadPopUpURL" windowState="<%= LiferayWindowState.EXCLUSIVE.toString()%>" copyCurrentRenderParameters="false">
	<liferay-portlet:param name="taskId" value="<%=taskId%>" />
...
<div style="width:45%; float:right; text-align:right;">
	...
	<input class="button01b" style="min-width:90px;display:none;" id="<portlet:namespace/>sampleFileBtn" value="<liferay-ui:message key='select-a-sample-file'/>" type="button"/>
	<input class="button01b" style="min-width:90px;" id="<portlet:namespace/>confirmBtn" value="<liferay-ui:message key='select-a-file'/>" type="button"/>
...
<div id="icebreaker-file-upload-dialog" title="<liferay-ui:message key='upload-a-file'/>" class="bigpopupbox" style="display:none;">
</div>
		...
		success: function(data) {
			...
			if(dataSize>0){
				for(var i=0 ; i<dataSize; i++ ){
					...
					if('<%=uploadDestFileName%>' == dataMap[i].fileName)
					{
						$tr.addClass("selected-tablebgtr"); 
						isChecked = true;
					}
					else if(i%2 == 1)
					{ 
					...
					$label = $("<label/>");
					$img = $("<img/>").attr("src","${contextPath}/images/fileselector/myfile-icon05.png").attr("width","14").attr("height","16");
					$checkBox = $("<input/>").attr("type","checkbox").attr("name","<portlet:namespace/>fileChk").attr("value",dataMap[i].fileId).css('visibility', 'hidden').prop("checked", isChecked);
					
					$fileName = $("<span/>").attr("id","fileDownLoad").text(dataMap[i].fileName);
					
					$label = $label.append("&nbsp;&nbsp;").append($img).append("&nbsp;").append($fileName).css("word-break","break-all").append($checkBox);
...
function <portlet:namespace/>initFileCheckEvent()
{
	$("input[name=<portlet:namespace/>fileChk]").click(function(){
    	...
    	var i = 0;
   		if(1 < checkedLength ){
   			alert( Liferay.Language.get('please-select-a-file'));
...
<script>
	...
	$("#<portlet:namespace/>sampleFileBtn").click(function() {
		...
		{
			alert(Liferay.Language.get('you-selected-the-sample-file'));
