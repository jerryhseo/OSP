...
// 	}
	...
	inputdeckFormData = OSPInputdeck.createForm();
	inputdeckFormData.setData(JsonData);
	var <portlet:namespace/>editor = OSPInputdeck.createEditor(inputdeckFormData, '<portlet:namespace/>', $("#<portlet:namespace/>variableTableTd"), Liferay.ThemeDisplay.getLanguageId());
	<portlet:namespace/>editor.showEditor();
...
// 		Liferay.Util.getOpener().fetchResult( taskId,portName, JSON.stringify(generateInputFile( inputdeckFormData )) );
		var JSONdata = JSON.stringify(inputdeckFormData.getActiveVariableFormattedInputs());
		Liferay.Util.getOpener().fetchResult( taskId,portName, JSONdata);
