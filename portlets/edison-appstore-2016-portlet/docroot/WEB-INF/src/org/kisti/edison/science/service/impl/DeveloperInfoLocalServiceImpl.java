...
public class DeveloperInfoLocalServiceImpl
	 ...
	 */
	public int getScienceAppDeveloperInfoCount(long userId) throws SystemException {
		int count = 0;
		if(userId != 0){			
			DeveloperInfo developerInfo =  developerInfoPersistence.fetchByPrimaryKey(userId);
...
//				resultRow.put("universityFieldNm", EdisonExpndoUtil.getCommonCdSearchFieldValue(developerInfo.getUniversityField(), EdisonExpando.CDNM + "_" + locale)); // 임시
	...
	public DeveloperInfo insertCustomDeveloperInfo(Map<String, Object> params) throws NoSuchDeveloperInfoException, NumberFormatException, SystemException {
		...
		if (userId != null && userId.length() > 0) {
			developerInfo = developerInfoPersistence.create(Long.parseLong(userId));
	...
	public DeveloperInfo updateCustomDeveloperInfo(Map<String, Object> params) throws NumberFormatException, SystemException, NoSuchModelException {
		...
		if (userId != null && userId.length() > 0) {
			developerInfo = developerInfoPersistence.findByPrimaryKey(Long.parseLong(userId));
	...
	public DeveloperInfo deleteCustomDeveloperInfo(Map<String, String> params) throws NoSuchDeveloperInfoException, NumberFormatException, SystemException {
		...
		if (userId != null && userId.length() > 0) {
			developerInfo = developerInfoPersistence.remove(Long.parseLong(userId));
		}
		return developerInfo;
	}
	
	public List<Map<String, Object>> getDeveloperRequestStatus(long portalGroupId, long userId, String requestStatus[], Locale locale, int begin, int end) {
		List<Map<String, Object>> returnList = new ArrayList<Map<String, Object>>();
		List<Object[]> resultList = developerInfoFinder.getDeveloperRequestStatus(userId, requestStatus, begin, end);
		if(resultList != null) {
			for (Object[] result : resultList) {
				...
				resultRow.put("useStart", developerInfo.getUseStart());
				resultRow.put("useEnd", developerInfo.getUseEnd());
				resultRow.put("groupId", portalGroupId);
				Group group;
				try {
	...
	public int getCountDeveloperRequestStatus(long userId, String requestStatus[]) {
		return developerInfoFinder.getCountDeveloperRequestStatus(userId, requestStatus);
	}
	...
	public void deleteWorkSpace(long userId, long groupId) throws SystemException, PortalException, SQLException, IOException{
		...
		List<DeveloperRequest> developerRequestList = developerRequestPersistence.findByUserId(userId);
		...
		List<DeveloperIp> developerIpList = developerIpPersistence.findByUserId(userId);
		...
		developerInfoPersistence.removeByUserId(userId);
