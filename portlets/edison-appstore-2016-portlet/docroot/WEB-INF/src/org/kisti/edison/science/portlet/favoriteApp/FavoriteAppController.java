...
public class FavoriteAppController {
	...
	public void favoriteAppList(ResourceRequest request, ResourceResponse response) throws IOException{
		try {
			...
			List<Map<String, Object>> favoriteAppList = ScienceAppFavoriteLocalServiceUtil.getFavoriteAppList(companyGroupId, groupId, locale, userId);

			List favoriteAppManualList =  new ArrayList();
			Map manualMap = null;
			for (int i = 0; i < favoriteAppList.size(); i++) {
				//solver_manual
				List solverManualList = EdisonFileUtil.getListEdisonFile(Long.parseLong(String.valueOf(favoriteAppList.get(i).get("groupId"))), "", String.valueOf(favoriteAppList.get(i).get("scienceAppId")), EdisonFileConstants.SCIENCE_APPS);
