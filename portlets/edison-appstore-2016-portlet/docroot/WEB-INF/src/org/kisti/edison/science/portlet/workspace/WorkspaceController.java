...
public class WorkspaceController {
	...
	public String view(RenderRequest request, RenderResponse response,ModelMap model) {
		try {
			...
			String workspacePopup = ParamUtil.getString(request, "workspacePopup", "");
			
			if(workspacePopup.equals("pop_up")){
				redirectName = "";
				redirectURL = "";
			}
			...
			model.addAttribute("workspacePopup", workspacePopup);
	...
	@RenderMapping(params = "myRender=workspaceRequest")
	public String view(RenderRequest request, ModelMap model) throws SystemException, IOException, PortalException, SQLException{
		...
		try{
			//보안서약서 셈플 파일 가지고 오기
			DLFolder folder = DLFolderLocalServiceUtil.getFolder(portalGroupId, DLFileEntryTypeConstants.FILE_ENTRY_TYPE_ID_BASIC_DOCUMENT, "WORKSPACE_SECURITY");
			List<DLFileEntry> dfeList = DLFileEntryLocalServiceUtil.getFileEntries(portalGroupId, folder.getFolderId());
		...
		if(delevoperMap != null){
			...
			try{
				//접속방법 매뉴얼 파일 가지고 오기
				DLFolder folder = DLFolderLocalServiceUtil.getFolder(portalGroupId, DLFileEntryTypeConstants.FILE_ENTRY_TYPE_ID_BASIC_DOCUMENT, "WORKSPACE_MANUAL");
				List<DLFileEntry> dfeList = DLFileEntryLocalServiceUtil.getFileEntries(portalGroupId, folder.getFolderId());
				List fileList = new ArrayList();
	...
	//		String universityNm = EdisonExpndoUtil.getCommonCdSearchFieldValue(university, field);
			...
			try {
				...
				EdisonFileUtil.insertWorkspaceDocFile(request, upload, developerInfo.getUserId(), portalGroupId, filePreFix, CustomUtil.strNull(String.valueOf(developerInfo.getUserId())), "addfile", CustomUtil.strNull(developerInfo.getScreenName()));
			...
			portletURL.setParameter("myRender", "workspaceRequest");
...
//		writer.close();
	...
	public String workspaceView(final RenderRequest request, ModelMap model) throws PortalException, SystemException, MalformedURLException, IOException, SQLException{
		...
		List fileList = EdisonFileUtil.getListWorkspaceDocFile(portalGroupId, filePreFix, CustomUtil.strNull(delevoperMap.get("userId")));
		String groupName = GetterUtil.get(CustomUtil.strNull(params.get("groupName")), "");
		if(groupName != null & !groupName.equals("")) {
			model.addAttribute("groupName", groupName);
		}
	    ...
	    try{
			...
			DLFolder folder = DLFolderLocalServiceUtil.getFolder(portalGroupId, DLFileEntryTypeConstants.FILE_ENTRY_TYPE_ID_BASIC_DOCUMENT, "WORKSPACE_SECURITY");
			List<DLFileEntry> dfeList = DLFileEntryLocalServiceUtil.getFileEntries(portalGroupId, folder.getFolderId());
			List samplefileList = new ArrayList();
			if(dfeList != null && dfeList.size() > 0){
				Map urlMap = null;
				for(int f=0;f < dfeList.size();f++){
					DLFileEntry dfe = dfeList.get(f);					
					...
					urlMap.put("fileUserId", String.valueOf(dfe.getUserId()));
					urlMap.put("fileExtension", String.valueOf(dfe.getExtension()));
					samplefileList.add(urlMap);					
				}
				model.addAttribute("securityFileList",samplefileList);
			}
		}catch (  NoSuchFolderException nsfe) {
			log.error(nsfe.toString());
		}
	...
	public void developerConfirm(ActionRequest request, ActionResponse response) throws IOException, SystemException, PortalException, SQLException, PortletModeException, WindowStateException{
		try{
			...
			if(!fileName.equals("") && fileName != null) {
				...
				try {
					EdisonFileUtil.deleteWorkspaceDocFile(portalGroupId, filePreFix, CustomUtil.strNull(String.valueOf(developerUser.getUserId())));
					EdisonFileUtil.insertWorkspaceDocFile(request, upload, developerUser.getUserId(), portalGroupId, filePreFix, CustomUtil.strNull(String.valueOf(developerUser.getUserId())), "addfile", CustomUtil.strNull(developerUser.getScreenName()));
				} catch (SQLException e) {
					e.printStackTrace();
				} catch (IOException e) {
					e.printStackTrace();
			...
			portletURL.setPortletMode(PortletMode.VIEW);
			portletURL.setParameter("myaction", "workspaceView");
			portletURL.setParameter("workspacePopup", workspacePopup);
			if(workspacePopup.equals("pop_up")){
				portletURL.setParameter("p_p_state", "pop_up");
			}else{
				portletURL.setParameter("p_p_state", "maximized");
			}	
			portletURL.setParameter("edionCopyParam", "true");
			portletURL.setParameter("redirectName", redirectName);
			portletURL.setParameter("redirectURL", redirectURL);
			portletURL.setParameter("userId",String.valueOf(developerUser.getUserId()));
...
//		List<Long> managerSolverIdList = UserGroupRoleCustomLocalServiceUtil.getCustomIdList(userId, groupId, solverManagerRoleId);
	...
	public void developerRequestStatus(ResourceRequest request, ResourceResponse response) throws IOException{
		try {
			...
			developerStatus = DeveloperInfoLocalServiceUtil.getDeveloperRequestStatus(portalGroupId, userId, null,locale, 0, 10);
