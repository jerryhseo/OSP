...
public class DeveloperInfoFinderImpl extends BasePersistenceImpl<DeveloperInfo> implements DeveloperInfoFinder{
	...
	/*개발자 요청 정보 조회 */
	public List<Object[]> getDeveloperRequestStatus(long userId, String requestStatus[], int begin, int end) {
		StringBuilder sqlSb = new StringBuilder();
		Session session = null;
	...
	public Object[] getCountRequestInfo(long groupId, String[] developerStatus, String virtualLabStatus,String libRequestStatus, String developerDeleteStatus) {
		...
		try {
			...
			Map params = new HashMap();
			String developerStatusStr = "";
			
			if(developerStatus != null && developerStatus.length > 0) {
				for(int i=0;i<developerStatus.length;i++){
					params.put("developerStatus"+i, developerStatus[i]);
				}
			}			
