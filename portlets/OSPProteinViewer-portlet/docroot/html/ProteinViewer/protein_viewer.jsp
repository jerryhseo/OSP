...
<div class="container-fluid osp-analyzer">
	...
	<div class="row-fluid frame">
		<iframe class ="col-sm-12 iframe-canvas" id="<portlet:namespace/>canvas" src="<%=request.getContextPath()%>/html/ProteinViewer/load_protein.jsp">
		</iframe>
	</div>
</div>
...
var <portlet:namespace/>fileExplorerId = "FileExplorer_WAR_OSPFileExplorerportlet_INSTANCE_pv";
...
function <portlet:namespace/>openServerFile(){
    ...
    if(!$.isEmptyObject(<portlet:namespace/>initData) && 
        <portlet:namespace/>initData.type_ !== 'url' &&
        <portlet:namespace/>initData.type_ !== 'context' ){
		...
		function(event){
			...
			reader.onload = function (e) {
				$('#<portlet:namespace/>canvas').each(function(){
							$(this).prop('contentWindow').loadProtein(e.target.result);
...
Liferay.on(
		...
		function(e){
			...
			$("#<portlet:namespace/>canvas").attr('src', '<%=request.getContextPath()%>/html/ProteinViewer/load_protein.jsp');
			<portlet:namespace/>initData = {};
...
function <portlet:namespace/>loadData( inputData ){
	AUI().use('liferay-portlet-url', function(a) {
		<portlet:namespace/>currentData = JSON.parse( JSON.stringify(inputData) );
		...
		$('#<portlet:namespace/>title').html('<h4>'+inputData.name_+'</h4>');
	});
}
	...
	setTimeout(
	    function(){
	    	...
	    	if (  iframeDoc.readyState  == 'complete' && iframe.contentWindow.loadProtein ) {
	   	    	iframe.contentWindow.loadProtein(url, $('#<portlet:namespace/>canvas').width(), $('#<portlet:namespace/>canvas').height() );
	    	} 
	    	else{
...
function <portlet:namespace/>getFirstFileName( argData ){
    ...
    if( inputData.type_=== 'folder' ){
    	inputData.parent_ = <portlet:namespace/>mergePath(inputData.parent_, inputData.name_);
    	inputData.name_ = '';
    }
    var data = {
