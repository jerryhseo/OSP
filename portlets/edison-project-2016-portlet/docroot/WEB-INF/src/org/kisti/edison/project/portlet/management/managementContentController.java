...
public class managementContentController {
	...
	public void myContentForProjectList(ResourceRequest request, ResourceResponse response){
		try{
			...
			List<Map<String, Object>> dataList = ContentLocalServiceUtil.getContentForProjectList(userId, searchText, projectCategoryId, themeDisplay.getLanguageId(), start, listSize, locale);
			...
			int totalCount = ContentLocalServiceUtil.getGeneralContentCountByGroupIdForProjectList(userId, searchText, projectCategoryId, themeDisplay.getLanguageId());
	...
	public void updateMyContentProjectCategory(ResourceRequest request, ResourceResponse response) throws IOException{
			...
			if(isRoleTure){
				...
				if(content != null){
					if(selectedCategoryId != 0){
						content.setProjectYn("Y");
					}
					content.setProjectId(selectedCategoryId);
