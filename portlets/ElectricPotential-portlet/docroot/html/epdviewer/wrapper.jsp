...
<div class="container-fluid osp-analyzer">
	...
	<div class="row-fluid frame">
		<iframe class="col-sm-12 iframe-canvas" id="<portlet:namespace/>canvas" src="<%=request.getContextPath()%>/html/epdviewer/epdviewer.jsp">
		</iframe>
	</div>
</div>
...
Liferay.on(
	'EP_DRAW_OBJECT',
	function( e ){
		console.log('EP_DRAW_OBJECT: ', e.data );
		var object = e.data;

		<portlet:namespace/>drawEPObject( object );
	}
);
...
Liferay.on(
       	'EP_MOVE_OBJECT',
       	function( e ){
       		console.log('EP_MOVE_OBJECT: ', e.data );
       		var object = e.data;

       		<portlet:namespace/>moveEPObject( object );
       	}
       );

Liferay.on(
          	'EP_CHANGE_TRANSPARENCY',
          	function( e ){
          		console.log('EP_CHANGE_TRANSPARENCY: ', e.data );
          		var object = e.data;

          		<portlet:namespace/>changeEPTransparency( object.shape, object.o );
          	}
          );

Liferay.on(
	'EP_DRAW_OBJECT_VOLUME',
	function( e ){
		console.log('EP_DRAW_OBJECT_VOLUME: ', e.data );
		var object = e.data;

		<portlet:namespace/>drawEPObjectVolume( object.shape, object.x, object.y, object.z, object.o, object.r );
	}
);
...
function <portlet:namespace/>drawEPObject( object ){
	setTimeout(
		function(){
			var iframe = document.getElementById('<portlet:namespace/>canvas');
			var iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
			if( iframeDoc.readyState  == 'complete' && iframe.contentWindow.displayObject ){
				iframe.contentWindow.displayObject( object.shape, object.x, object.y, object.z, object.r, object.o, false ); 
			}
			else{
				<portlet:namespace/>drawEPObject( object );
			}
...
function <portlet:namespace/>moveEPObject( object ){
	setTimeout(
		function(){
			var iframe = document.getElementById('<portlet:namespace/>canvas');
			var iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
			if( iframeDoc.readyState  == 'complete' && iframe.contentWindow.moveEPObject ){
				iframe.contentWindow.moveEPObject( object.shape, object.x, object.y, object.z ); 
			}
			else{
				<portlet:namespace/>drawEPObject( object );
			}
...
function <portlet:namespace/>drawEPObjectVolume( shape, x, y, z, o, r ){
	setTimeout(
		function(){
			var iframe = document.getElementById('<portlet:namespace/>canvas');
			var iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
			if( iframeDoc.readyState  == 'complete' && iframe.contentWindow.displayVolume ){
				iframe.contentWindow.displayVolume( shape, x, y, z, o, r ); 
			}
			else{
				<portlet:namespace/>drawEPObjectVolume( shape, x, y, z, o, r );
			}
		}, 
		10
	);
