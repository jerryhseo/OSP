...
public  class EdisonFileUtil{
	 ...
	 * DLfile 복사
	 * @param request
	 * @param sourceFileEntryId - 복사할 대상의 fileEntryId
	 * @param preFix			
	 * @param fileGroupSeq
	 * @param fileCategory
	 * @return
	 ...
	 * @throws IOException
	 */
	public static long copyDlFile(PortletRequest request,long sourceFileEntryId,String preFix, String fileGroupSeq, String fileCategory) throws PortalException, SystemException, IOException{
		DLFileEntry sourceDlFileEntry =  DLFileEntryLocalServiceUtil.getFileEntry(sourceFileEntryId);
		
		long groupId = sourceDlFileEntry.getGroupId();
		long userId = sourceDlFileEntry.getUserId();
		...
		InputStream sourceFileObj = sourceDlFileEntry.getContentStream();
		File tempfile = null;//FileUtil.createTempFile(sourceFileObj);
		
		try {
			tempfile = FileUtil.createTempFile(sourceFileObj);
		...
		FileEntry targetFileEntry = null;
		String fileName = sourceDlFileEntry.getTitle();
		
		try {
			targetFileEntry = DLAppLocalServiceUtil.addFileEntry( 
												userId,	groupId,	folder.getFolderId(),
												fileName,
												sourceDlFileEntry.getMimeType(),
												fileName, 
												fileName, "",
												FileUtil.getBytes(tempfile), 
												sctx
												);
			...
			targetFileEntry = DLAppLocalServiceUtil.addFileEntry(
												...
												fileName, 
												sourceDlFileEntry.getMimeType(),
												onlyFileName+"_"+CustomUtil.getRandomString(3)+ext, 
												fileName, "", 
												FileUtil.getBytes(tempfile), 
												sctx
												);
		...
		if(tempfile!=null){if(tempfile.exists()){tempfile.delete();}}
		
		return targetFileEntry.getFileEntryId();
	}
		...
		if(fileNames != null){
			...
			for(int i=0;i<fileNames.length;i++){				
				if (Validator.isNotNull(fileNames[i])) {
					...
					InputStream fileObj = inputStream[i];
					File tempfile = null;//FileUtil.createTempFile(fileObj);
	...
	/**
	 * 경로의 폴더 생성
	 * /groupId_EDISON_FILE/fileCategory/groupId_fileGroupSeq
	 * @param request
	 * @param userId
	 * @param groupId
	 * @param preFix
	 ...
	 * @throws SystemException
	 */
	public static DLFolder createEdisonDLFolder(PortletRequest request, long userId, long groupId, String fileGroupSeq, String fileCategory) throws PortalException, SystemException{
