 ...
 *
 */
public class TokenProviderUtil_fileTest{
	...
	public static String getIcebreakerMajinURL(String icebreakerUrl){
		
		if(icebreakerUrl != null && !"".equals(icebreakerUrl)){
			if(icebreakerUrl.contains("/test")){
				icebreakerUrl = icebreakerUrl.replaceAll("/test", "/majin");
			}
		}
		return icebreakerUrl;
	}
	public static IcebreakerVcToken getVcToken(long groupId, String icebreakUserId, String icebreakerUserPwd) throws MalformedURLException, IOException, ParseException, PortalException, SystemException  {
		...
		String icebreakerUrl = getIcebreakerMajinURL(CustomUtil.strNull(thisGroup.getExpandoBridge().getAttribute(EdisonExpando.SITE_ICEBREAKER_URL)));
