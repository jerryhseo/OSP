...
public final class EdisonTempUserUtil {
    ...
    public static User addUser(ResourceRequest actionRequest, Map userTemp) throws Exception {
		...
		if(user.getExpandoBridge().hasAttribute(EdisonExpando.USER_VISIT_SITE)) user.getExpandoBridge().setAttribute(EdisonExpando.USER_VISIT_SITE,CustomUtil.strNull(userTemp.get("groupName")));
		if(user.getExpandoBridge().hasAttribute(EdisonExpando.TEMP_USER_JOIN_FROM_GROUP)) user.getExpandoBridge().setAttribute(EdisonExpando.TEMP_USER_JOIN_FROM_GROUP,CustomUtil.strNull(userTemp.get("groupId")));
