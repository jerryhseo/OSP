var UIPanelExecutor = (function(namespace, $, designer, executor, toastr) {
	...
	var PANEL_DATA = {
		...
		"open" : {
			...
			"header" : {
				...
				"theads" : [ "Title", "Description", "App Name", "Version",
						"Status", "Copied from", "Created" ]
			},
			"form" : {},
			"footer" : {
		...
		"status" : {
			...
			"form" : {}
		/*
		 * , "footer":{ "id": "tpl-menu-panel-pagination", "btns" : [ { "name":
		 * "Open", "func": openWorkflow }, { "name": "Rename", "func":
		 * renameWorkflow }, { "name": "Duplicate", "func": duplicateWorkflow }, {
	...
	$(JQ_PORTLET_BOUNDARY_ID + " .sidbar-run-btn").click(
			function(e) {
				e.preventDefault();
				...
				if (!PANEL_DATA.setting.form.simulationId) {
					...
					$(
							JQ_PORTLET_BOUNDARY_ID
									+ " .sidebar-btn[data-btn-type='new']")
							.click();
					return false;
				}
				if (btnType === "run") {
	...
	function run() {
		...
		executor.getWorkflowInstance(simulationId, function(
	...
	function rerun() {
		if ($(".wf-box.reset").length) {
			var simulationId = PANEL_DATA.setting.form.simulationId;
			executor
					.reRunWorkflowInstance(
							simulationId,
							designer.getWorkflowDefinition(designer
									.getCurrentJsPlumbInstance()),
							function(workflowInstance) {
								var ibToken = getIcebreakerAccessToken();
								executor.runWorkflowInstance(
										workflowInstance.simulationId, ibToken);
								toastr["success"]("",
										var_success_run_workflow_message);
							});
		} else {
	...
	function pause() {
		...
		executor.getWorkflowInstance(simulationId, function(
			...
			if (currentWorkflowInstance.status === WF_STATUS_CODE.RUNNING) {
				executor.pauseWorkflowInstance(simulationId, function(
						workflowInstance) {
	...
	function restart() {
		...
		executor.getWorkflowInstance(simulationId, function(
			...
			if (currentWorkflowInstance.status === WF_STATUS_CODE.PAUSED) {
				executor.resumeWorkflowInstance(simulationId, function(
						workflowInstance) {
						...
						function(workflowStatus) {
							...
							for (var i = 0; i < simulations.length; i++) {
								...
								if (simulations[i].jobs[0].submittedTime) {
									tableSimulation.submittedTime = $.format
											.date(
													new Date(
								...
								if (simulations[i].jobs[0].endTime) {
									tableSimulation.endTime = $.format
											.date(
													new Date(
							...
							var tbodyTemplate = '{{#rows}}' + '    <tr>'
									+ '        <td>{{title}}</td>'
									+ '        <td>{{status}}</td>'
									+ '        <td>{{submittedTime}}</td>'
									+ '        <td>{{endTime}}</td>'
									+ '    </tr>' + '{{/rows}}';
							tbodyTemplate += '{{^rows}}' + '    <tr>'
	...
	function status(that, btnType) {
		...
		$("#" + namespace + "menu-panel-box").empty().mustache(
				'tpl-menu-panel-box', templateData);
		$("#" + namespace + "menu-panel-box .box-body").mustache(
				templateData.body, templateData);
		if (templateData.header) {
			var boxTitleSelecotr = "#"
					+ namespace
					+ "menu-panel-box .box-header.with-border.header-inner > .box-title";
			$(boxTitleSelecotr).replaceWith(
					$.Mustache.render(templateData.header.id, templateData));
		...
		if ($(that).parent("li").hasClass("menu-open")) {
			...
			$(JQ_PORTLET_BOUNDARY_ID + " .sidebar-menu > li").removeClass(
					"menu-open");
		} else {
			$(that).parent("li").addClass("menu-open")
			$(".menu-panel").show('slide', {
				direction : 'left'
			}, 500);
		}
	...
	$(JQ_PORTLET_BOUNDARY_ID + " .sidebar-btn")
			.click(
					function(e) {
						e.preventDefault();
						var btnType = $(this).attr("data-btn-type");
						...
						if (templateData) {
							...
							$("#" + namespace + "menu-panel-box").show();
							$("#" + namespace + "menu-panel-box").empty()
									.mustache('tpl-menu-panel-box',
											templateData);
							$("#" + namespace + "menu-panel-box .box-body")
									.mustache(templateData.body, templateData);
							if (templateData.header) {
								var boxTitleSelecotr = "#"
										+ namespace
										+ "menu-panel-box .box-header.with-border.header-inner > .box-title";
								$(boxTitleSelecotr).replaceWith(
										$.Mustache.render(
												templateData.header.id,
												templateData));
								var _delay600 = _instantDelay(600);
								$(boxTitleSelecotr + " > .search-input").keyup(
										function(e) {
											_delay600(openPaginatedSimulations,
													btnType);
										});
							}
							...
							$(".menu-panel .menu-panel-close").click(
									function(e) {
										e.preventDefault();
										closePanel();
									});

							$("#" + namespace + "menu-panel-box .data-binded")
									.change(
											function(e) {
												var thisValue = $(this).val();
												var thisName = $(this).attr(
														"name");
												templateData.form[thisName] = thisValue;
											});

							$("#" + namespace + "menu-panel-box .func")
									.each(
											function(_) {
												var thisName = $(this).attr(
														"name");
												if (templateData.btn
														&& templateData.btn[thisName]) {
													$(this)
															.click(
																	function(e) {
																		templateData.btn[thisName]
