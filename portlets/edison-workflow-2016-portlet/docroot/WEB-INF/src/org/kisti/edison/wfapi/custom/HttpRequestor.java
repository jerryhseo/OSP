...
public final class HttpRequestor{
  ...
  public void addFile(String parameterName, InputStream parameterValue){
    // paramterValue가 null일 경우 NullFile을 삽입한다.
    if(parameterValue == null){
      list.add(parameterName);
      list.add(new NullFile());
    ...
    try{
      ...
      for(int i = 0; i < obj.length; i += 2){
        ...
        if(obj[i + 1] instanceof String){
          ...
          }else if(obj[i + 1] instanceof InputStream){
            BufferedInputStream is = null;
            try{
              is = new BufferedInputStream((InputStream) obj[i + 1]);
              byte[] fileBuffer = new byte[1024 * 8]; // 8k
              int len = -1;
              while((len = is.read(fileBuffer)) != -1){
                out.write(fileBuffer, 0, len);
      ...
      if(conn.getResponseCode() == 201){
        ...
        System.out.println(
            "Failed HttpRequestor [ sendMultipartPost ] : BAD REQUEST : wrong body content - HTTP error code : "
                + conn.getResponseCode());
      }else{
        System.out.println(
            "Failed HttpRequestor [ sendMultipartPost ] : ETC : etc error - HTTP error code : "
                + conn.getResponseCode());
      }
