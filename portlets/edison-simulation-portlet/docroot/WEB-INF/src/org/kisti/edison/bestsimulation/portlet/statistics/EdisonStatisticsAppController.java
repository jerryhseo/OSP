	...
	public void getStatisticsSw(ResourceRequest request, ResourceResponse response) throws PortalException, SystemException, ParseException, IOException{
		Map params = RequestUtil.getParameterMap(request);
			...
			if(parentGroupId == 0){//포탈
				tableOrganigationList  = SimulationJobLocalServiceUtil.getStatisticsSwTableOrganigation(companyGroupId, visitSite, locale, columnId, startDt, endDt, true);
				barChartDateList  = SimulationJobLocalServiceUtil.getStatisticsSwBarChartDate(companyGroupId, visitSite, locale, columnId, startDt, endDt, true);
			}else{
				tableOrganigationList  = SimulationJobLocalServiceUtil.getStatisticsSwTableOrganigation(companyGroupId, groupId, locale, columnId, startDt, endDt, true);
				barChartDateList  = SimulationJobLocalServiceUtil.getStatisticsSwBarChartDate(companyGroupId, groupId, locale, columnId, startDt, endDt, true);
			}
