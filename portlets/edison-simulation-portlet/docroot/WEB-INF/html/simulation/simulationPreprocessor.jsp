	 	...
	 	for (var i = 0; i < portMapList.length; i++) {
			...
			if(typeof simulationJobData != "undefined" && typeof simulationJobData[portMap.portName] != "undefined") {
				if(portMap.editorType == "Inputdeck") {
					...
					if(typeof simulationJobData[portMap.portName] === "object") {
	 					portData = JSON.stringify(simulationJobData[portMap.portName]);
					}else{
			...
			$("#preprocessorDiv").append('<div style="display:none;" id="<portlet:namespace/>' + portMap.portName + '" data-portTypeId=\'' + portMap.port_type_id + '\'">'+ portData +'</div>')
...
function popupDataInfo(id) {
	...
	var content = $('#<portlet:namespace/>' + id).text();
	...
	if(type == "Inputdeck") {
		...
		$('#<portlet:namespace/>jobparameter-dialog-content').append(parameterDataContent);
...
function preProcessorCheck(){
	...
	if(inputPortJson != "") {
		...
		for (var i = 0; i < portNameList.length; i++) {
			...
			if(mandatory == true) {
				var message = $('#<portlet:namespace/>' + portName).text();
...
function openEditorWindow(portletId, portletTitle, taskId, portName, token, plid, editorType, sampleFilePath)
{
	...
	var portData = $("#<portlet:namespace/>" + portName).text();
	...
	var renderURL = Liferay.PortletURL.createRenderURL();
  	...
  	if(editorType == "Inputdeck") {
	  	...
	  	if(portType == "Inputdeck") {
		  	portData = ((portData == "" || typeof portData == "undefined") ?  "" : JSON.stringify(JSON.parse(portData)[0]));
	  	}
	  	
		/////////////input deck ajax로 portlet session set
		var searchData = {
	  			"<portlet:namespace/>portData":portData,
				"<portlet:namespace/>dialogId":dialogId
		};
		jQuery.ajax({
			type: "POST",
			url: "<%=createPortDataSessionURL%>",
			data  : searchData,
			success: function(data) {
...
function openWindow(renderURL, dialogId)
{
