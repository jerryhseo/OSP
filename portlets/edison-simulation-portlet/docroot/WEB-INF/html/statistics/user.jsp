...
// 		$("#totalSpan").html("<b>"+totalCnt+" <liferay-ui:message key='edison-search-article'/></b>");
	}else{
		$("#totalSpan").html("<b>0 <liferay-ui:message key='edison-search-article'/></b>");
		$("<tr/>").append(
							$("<td/>").addClass("TC").attr("colspan","3").html('<liferay-ui:message key="edison-there-are-no-data" />')
	...
	if(pieChartOrganigationList != null){
		...
		if(pieChartOrganigationList.length > 0){
			for(var c=0; c<pieChartOrganigationList.length; c++){			
				...
				var cnt = pieChartOrganigationList[c].userCnt;	
...
function dtCheckBox(){
	...
	AUI().ready(function() {
		$("#<portlet:namespace/>startDt").datepicker({
			...
			title: {
					text: '<liferay-ui:message key="edison-appstore-student-sts"/>'
			    },
			plotOptions: {
				pie: {
			...
			title: {
					text: '<liferay-ui:message key="edison-appstore-month-student-sts"/>'
			    },
			yAxis: {
				min: 0,
				title: {
