...
<!-- JQuery -->
<link href="<%= request.getContextPath() %>/css/osp-analyzer.css" rel="stylesheet" type="text/css">
...
<div class="container-fluid osp-analyzer">
	...
	<div class="row-fluid frame">
		<iframe class="col-sm-12 iframe-canvas" id="<portlet:namespace/>canvas" src="<%= request.getContextPath() %>/html/sc3dviewer/sc3d_viewer.jsp">
		</iframe>
	</div>
</div>
...
var atomTransitorAnalyzerImagePath = "<%= request.getContextPath() %>/html/sc3dviewer/pallet.png";
...
Liferay.on(
		...
		function(e) {
			...
			$("#<portlet:namespace/>canvas").attr('src', '<%= request.getContextPath() %>/html/sc3dviewer/sc3d_viewer.jsp');
...
Liferay.on(
	   	'Receive_IniStruc_from_Editor',
	   	function( e ) {
	   		console.log('Receive_IniStruc_from_Editor: ', e.data );
	   		var object = e.data;

	   		//alert(object);
	   		<portlet:namespace/>Send_IniStruc_to_S( object );
	   	}
	   );
...
function <portlet:namespace/>Send_IniStruc_to_S( data )
{
	...
	setTimeout(
			function() {
				var iframe = document.getElementById('<portlet:namespace/>canvas');
				var iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
				if ( iframeDoc.readyState == 'complete' && iframe.contentWindow.load_Struc ) {
					iframe.contentWindow.load_Struc( data );
				}
				else {
					<portlet:namespace/>Send_IniStruc_to_S( data );
				}
			},
			10
		);
