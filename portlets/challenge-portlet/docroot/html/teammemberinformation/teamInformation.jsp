...
<%
	...
	String currentUserSN = ParamUtil.getString(request, "currentUserSN");
	...
	System.out.println("current user id  :  ["+currentUserSN+"]");
	...
	if(leaderMember.getScreenName()!=null){
		System.out.println("test current is leader check 1");
		if(leaderMember.getScreenName().equals(currentUserSN)){
			System.out.println("test current is leader check 2");
			curretIsLeader = true;}
	}
	System.out.println("=========================\n\n\n\n\n\n\n");
%>
...
<portlet:actionURL name="updateTeam" var="updateTeam" windowState="normal">
	<portlet:param name="mvcPath" value="/html/teammemberinformation/view.jsp" />
	<portlet:param name="chTeamID" value="<%=String.valueOf(chTeamID) %>"/>
	<portlet:param name="childChallengeID" value="<%=String.valueOf(challengeid) %>"/>
</portlet:actionURL>
	...
	<table class="table">
		...
		<tr>
			...
			<td><select name="<portlet:namespace/>inputUserGrade" id="<portlet:namespace/>inputUserGrade">
				...
				<script>
					$('#<portlet:namespace/>inputUserGrade').val("<%=leaderMember.getGrade()%>");
				</script>
			</td>
		</tr>
		<tr>
			<td><label><liferay-ui:message key="challenge-team-name"></liferay-ui:message></label></td>
			<td colspan="3"><liferay-ui:input-localized style="width:80%" required="required" autofocus="autofocus" name="inputTeamName" xml="<%=myTeam.getTeamName() %>"></liferay-ui:input-localized></td>
		</tr>
		<tr>
			<td><label><%=LanguageUtil.get(themeDisplay.getLocale(), "challenge-user-papername") %></label></td>
			<td colspan="3"><liferay-ui:input-localized  type="textarea" style="width:80%" name="inputPaperName" xml="<%=myTeam.getPaperName() %>"></liferay-ui:input-localized></td>
		</tr>
		<tr>
			<td><label><%=LanguageUtil.get(themeDisplay.getLocale(), "challenge-user-paperabstract") %></label></td>
			<td colspan="3"><liferay-ui:input-localized type="textarea" rows="10" style="width:80%" name="inputpaperAbstract" xml="<%=myTeam.getPaperAbstract() %>"></liferay-ui:input-localized></td>
		</tr>
		
		<tr rowspan="2">
			<td><label><%=LanguageUtil.get(themeDisplay.getLocale(), "challenge-user-papersubmission") %></label></td>
			<td colspan="2"><input type="file" name="<portlet:namespace/>papersubmission" accept=".doc, .docx" disabled></input>
				<label>.doc, .docx</label>
			</td>
			<td>				
				<portlet:resourceURL var="getMyTeamPaperFile">
					<portlet:param name="type" value="paperFile"/>
					<portlet:param name="chTeamID" value="<%=String.valueOf(myTeam.getPrimaryKey())  %>" />
				</portlet:resourceURL> 
				<a href="<%=getMyTeamPaperFile %>" >Download : <%=myTeam.getPaperstatus()?myTeam.getPaperFileName():"" %></a>
			</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			<td colspan="2">
				<input type="file" name="<portlet:namespace/>papersubmissionPDF" accept=".pdf" disabled></input><label>.pdf </label>
			</td>
			<td>				
				...
				<a href="<%=getMyTeamPaperFilePDF %>" >Download : <%=myTeam.getPaperPDFstatus()?myTeam.getPaperPDFFileName():"" %></a>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<a href="<%=getMyTeamPresentationFile %>">Download : <%=myTeam.getPresentationstatus()?myTeam.getPresentationFileName():"" %></a>
			</td>
		</tr>
			...
			<div class="control-group">
				<div class="controls">
					<c:choose>
						...
						<c:otherwise>
							<button class="btn" onclick="javascript: location.href='<%=homeURL.toString() %>';"><%=LanguageUtil.get(themeDisplay.getLocale(), "challenge-home") %></button>
						</c:otherwise>
					</c:choose>	
				</div>
			</div>
