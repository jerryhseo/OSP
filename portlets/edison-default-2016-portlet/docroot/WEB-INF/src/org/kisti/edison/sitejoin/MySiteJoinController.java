...
public class MySiteJoinController {
	...
	public String view(RenderRequest request, RenderResponse response, ModelMap model) throws ParseException {
		...
		try {
			...
			List<Group> childStieGroupList = new ArrayList<Group>();
			if(parentGroupId == 0){
				childStieGroupList=ListUtil.sort(GroupLocalServiceUtil.getGroups(themeDisplay.getCompanyId(), PortalUtil.getScopeGroupId(request), true),sort);
			}else{
				childStieGroupList.add(GroupLocalServiceUtil.getGroup(groupId));
			}
			...
			for(Group childSite : childStieGroupList){
				...
				String friendlyUrl = themeDisplay.getPortalURL() + themeDisplay.getPathFriendlyURLPublic() + childSite.getFriendlyURL();
				userSiteGroup.put("friendlyUrl", friendlyUrl);
				...
				userSiteGroup.put("groupAssetEntry", AssetEntryLocalServiceUtil.getEntry(Group.class.getName(), childSite.getGroupId()));
	...
	public void deleteAuthManagerId(ResourceRequest request, ResourceResponse response){
		try{
			...
			if(groupId != 0 && userId != 0){
				...
				DeveloperRequestLocalServiceUtil.updateDeveloperRequestStatus(userId, "1202007");
			}
	...
	@ActionMapping(params="myaction=updateSiteAssetEntryVisibleStatus")
	public void updateSiteAssetEntryVisibleStatus(ActionRequest request, ActionResponse response) {
		try {
			long groupId = ParamUtil.getLong(request, "groupId");
			boolean updateStatus = ParamUtil.getBoolean(request, "siteVisibleStatus", false);
			
			/* updateStatus 가 true 이면 활성화, false 이면 비활성화*/
			
			AssetEntry assetEntry = AssetEntryLocalServiceUtil.getEntry(Group.class.getName(), groupId);
			if(assetEntry.isVisible() != updateStatus){
				assetEntry = AssetEntryLocalServiceUtil.updateVisible(Group.class.getName(), groupId, updateStatus);
			}
