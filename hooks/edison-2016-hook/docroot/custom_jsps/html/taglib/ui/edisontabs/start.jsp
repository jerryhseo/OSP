...
// minwidth
int minwidth = GetterUtil.getInteger(request.getAttribute("liferay-ui:tabs:minwidth"));
...
<c:if test="<%= names.length > 0 %>">
	...
	for (int i = 0; i < values.length; i++) {
		...
		if (Validator.isNotNull(onClick)) {
			if (refresh) {
				...
				curOnClick = "Liferay.Portal.Tabs.show('" + namespace + param + "', " + namesJS + ", '" + UnicodeFormatter.toString(values[i]) + "', " + onClick + ");";
				curURL = "javascript:;";
			}
		}
		...
		String aCssClassName = StringUtil.toUpperCase(names[i]);
	...
	%>
		...
		<c:choose>
			...
			<c:otherwise>
				<li class="<%= cssClassName %>" id="<%= namespace %><%= param %><%= StringUtil.toCharCode(values[i]) %>TabsId" style="min-width: <%=minwidth%>px">
					<a class = "<%=aCssClassName%>" href="<%= Validator.isNotNull(curURL) ? curURL : "javascript:;" %>" onClick="<%= Validator.isNotNull(curOnClick) ? curOnClick : StringPool.BLANK %>">
						<%= LanguageUtil.get(pageContext, StringUtil.toUpperCase(names[i])) %>
					</a>
				</li>
			</c:otherwise>
		</c:choose>
